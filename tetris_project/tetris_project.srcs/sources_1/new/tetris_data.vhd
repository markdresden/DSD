...
tetrimino_data.tiles(i).y + 1).state = "01") then
 can_fall <= '0';
 end if;

 if (tetrimino_data.tiles(i).x = 0) or
 (board_data(tetrimino_data.tiles(i).x - 1,
tetrimino_data.tiles(i).y).state = "01") then
 can_move_left <= '0';
 end if;
...
rotate_function(tetrimino_data).tiles(i).y).state = "01") then
 can_rotate <= '0';
 end if;
...
for i in 0 to 3 loop
for j in 1 to BOARD_HEIGHT - 1 loop
if (board_data(tetrimino_data.tiles(i).x,tetrimino_data.tiles(i).y + j).state = "01" or (tetrimino_data.tiles(i).y + j >= BOARD_HEIGHT)) then distances(i) := j-1;
else exit;
end if;
end loop;
end loop;

if (distances(0) <= distances(1) and distances(0) <= distances(2) and distances(0) <= distances(3)) then
minDistance := distances(0);
elsif (distances(1) <= distances(2) and distances (2) <= distances(3)) then
minDistance := distances(1);
elsif (distances(2) <= distances(3)) then
